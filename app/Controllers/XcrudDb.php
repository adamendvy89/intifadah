...
class XcrudDb
{
    ...
    public static function get_instance($params = false)
    {
        ...
        if (!isset(self::$_instance[$instance_name]) or null === self::$_instance[$instance_name])
        {
            if (!is_array($params))
            {
                $dbuser = XcrudConfig::$dbuser;
                $dbpass = XcrudConfig::$dbpass;
                $dbname = XcrudConfig::$dbname;
                $dbhost = XcrudConfig::$dbhost;
                $dbencoding = XcrudConfig::$dbencoding;
            }
    ...
    private function __construct($dbuser, $dbpass, $dbname, $dbhost, $dbencoding)
    {
        ...
        if (XcrudConfig::$db_time_zone)
            $this->connect->query('SET time_zone = \'' . XcrudConfig::$db_time_zone . '\'');
    }
