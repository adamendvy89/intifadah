...
use App\Controllers\XcrudConfig;
use App\Controllers\XcrudDb;
use App\Controllers\XcrudXss;
use App\Controllers\XcrudFunction;
...
//require (XCRUD_PATH . '/XcrudConfig.php'); // configuration
//require (XCRUD_PATH . '/XcrudDb.php'); // database class
mb_internal_encoding(XcrudConfig::$mbencoding); // sets multibyte encoding globaly
...
class Xcrud
{
    ...
    public function __construct()
    {
        ...
        $this->language = XcrudConfig::$language;
        ...
        $this->prefix = XcrudConfig::$dbprefix;
    ...
    protected static function init_prepare($method = false)
    {
        ...
        {
            case 'post':
                $sess_name = (XcrudConfig::$dynamic_session && isset($_POST['xcrud']['sess_name']) && $_POST['xcrud']['sess_name']) ? $_POST['xcrud']['sess_name'] :
                    XcrudConfig::$sess_name;
                break;
            case 'get':
                $sess_name = (XcrudConfig::$dynamic_session && isset($_GET['xcrud']['sess_name']) && $_GET['xcrud']['sess_name']) ? $_GET['xcrud']['sess_name'] :
                    XcrudConfig::$sess_name;
                break;
            default:
                $sess_name = XcrudConfig::$sess_name;
                break;
        }
    ...
    public static function session_start($sess_name = false)
    {
        ...
        if (!session_id() && !XcrudConfig::$external_session && !XcrudConfig::$alt_session)
        {
            if (!headers_sent())
            {
                session_name($sess_name);
                session_cache_expire(XcrudConfig::$sess_expire);
        ...
        if (XcrudConfig::$alt_session)
        {
            if (!headers_sent())
            {
                ...
                setcookie($sess_name, $this->sess_id, time() + XcrudConfig::$alt_lifetime * 60, '/');
            }
            else
    ...
    protected function render_custom_datagrid()
    {
        ...
        $db = XcrudDb::get_instance($this->connection);
        ...
        if (!$this->primary_val)
        {
            ...
            if ($this->alert_create)
            {
                foreach ($this->alert_create as $alert)
                {
                    ...
                    $this->send_email($postdata[$alert['column']], $alert['subject'], $message, $alert['cc'], XcrudConfig::$email_enable_html);
                }
            }
    ...
    protected function _render_list()
    {
        ...
        $mode = 'list';
        $view_file = XCRUD_PATH . '/' . XcrudConfig::$themes_path . '/' . $this->theme . '/' . $this->load_view['list'];
    ...
    protected function _render_details($mode)
    {
        ...
        */
        ...
        $view_file = XCRUD_PATH . '/' . XcrudConfig::$themes_path . '/' . $this->theme . '/' . $this->load_view[$mode];
        $this->check_file($view_file, 'render');
        ob_start();
        include ($view_file);
    ...
    protected function _cut($string, $field, $wordsafe = true, $dots = true)
    {
        ...
        $string = html_entity_decode($string, ENT_QUOTES, XcrudConfig::$mbencoding);
        ...
        $slen = mb_strlen($strip_string, XcrudConfig::$mbencoding);
        if ($slen <= $len)
        {
            return $this->output_string($string, $this->strip_tags, $safe);
        }
        ...
        {
            $end = $len;
            while ((mb_substr($strip_string, --$len, 1, XcrudConfig::$mbencoding) != ' ') && ($len > 0))
            {
            }
            if ($len == 0)
            {
            ...
            return $this->output_string(mb_substr($strip_string, 0, $len, XcrudConfig::$mbencoding), false, $safe) . ($dots ?
                '&#133;' : '');
        }
        return $this->output_string(mb_substr($strip_string, 0, $len, XcrudConfig::$mbencoding), false, $safe) . ($dots ?
            '&#133;' : '');
    ...
    {
        return mb_convert_case(str_replace('_', ' ', $text), MB_CASE_TITLE, XcrudConfig::$mbencoding);
    }
    ...
    public function _export_vars()
    {
        ...
        if ($this->instance_count == 1 && !$this->ajax_request)
        {
            if (isset($_SESSION['xcrud_session']) && $_SESSION['xcrud_session'])
            {
                ...
                { // workaround on some servers session duplication
                    ...
                    if ($time > $old_time + XcrudConfig::$autoclean_timeout) // autocleaner
        ...
        if (XcrudConfig::$alt_session)
        {
            ...
            if (class_exists('Memcache'))
            {
                ...
                $res = $mc->set($this->sess_id, $data, false, XcrudConfig::$alt_lifetime * 60);
            }
            elseif (class_exists('Memcached'))
            {
                $mc = new Memcached();
                $mc->connect(XcrudConfig::$mc_host, XcrudConfig::$mc_port);
                $res = $mc->set($this->sess_id, $data, XcrudConfig::$alt_lifetime * 60);
            }
            else
            {
    ...
    protected function _compare($val1, $operator, $val2)
    {
        switch ($operator)
        {
            ...
            case '!=':
                return ($val1 != $val2) ? true : false;
            case '^=':
                return (mb_strpos($val1, $val2, 0, XcrudConfig::$mbencoding) === 0) ? true : false;
            case '$=':
                return (mb_strpos($val1, $val2, 0, XcrudConfig::$mbencoding) == (mb_strlen($val1, XcrudConfig::$mbencoding) -
                    mb_strlen($val2, XcrudConfig::$mbencoding))) ? true : false;
            case '~=':
                return (mb_strpos($val1, $val2, 0, XcrudConfig::$mbencoding) !== false) ? true : false;
            default:
                return false;
        }
    }
    ...
    {
        return htmlspecialchars((string )$text, ENT_QUOTES, XcrudConfig::$mbencoding);
    }
    ...
    {
        if (is_file(XCRUD_PATH . '/' . XcrudConfig::$lang_path . '/' . $this->language . '.ini'))
            $this->lang_arr = parse_ini_file(XCRUD_PATH . '/' . XcrudConfig::$lang_path . '/' . $this->language . '.ini');
        elseif (is_file(XCRUD_PATH . '/' . XcrudConfig::$lang_path . '/en.ini'))
            $this->lang_arr = parse_ini_file(XCRUD_PATH . '/' . XcrudConfig::$lang_path . '/en.ini');
        if ($this->set_lang)
        {
    ...
    {
        if (is_file(XCRUD_PATH . '/' . XcrudConfig::$lang_path . '/' . XcrudConfig::$language . '.ini'))
            return parse_ini_file(XCRUD_PATH . '/' . XcrudConfig::$lang_path . '/' . XcrudConfig::$language . '.ini');
        elseif (is_file(XCRUD_PATH . '/' . XcrudConfig::$lang_path . '/en.ini'))
            $this->lang_arr = parse_ini_file(XCRUD_PATH . '/' . XcrudConfig::$lang_path . '/en.ini');
    }
    protected function _get_theme_config()
    { // loads theme configuration from ini file
        if (is_file(XCRUD_PATH . '/' . XcrudConfig::$themes_path . '/' . $this->theme . '/xcrud.ini'))
            $this->theme_config = parse_ini_file(XCRUD_PATH . '/' . XcrudConfig::$themes_path . '/' . $this->theme . '/xcrud.ini');
        else
            self::error('xcrud.ini does not exist in your theme folder');
    }
    protected function lang($text = '')
    {
        $langtext = mb_convert_case($text, MB_CASE_LOWER, XcrudConfig::$mbencoding);
        return htmlspecialchars((isset($this->lang_arr[$langtext]) ? $this->lang_arr[$langtext] : $text), ENT_QUOTES,
            XcrudConfig::$mbencoding);
    }
    protected function theme_config($text = '')
    {
        $text = mb_convert_case($text, MB_CASE_LOWER, XcrudConfig::$mbencoding);
        return htmlspecialchars((isset($this->theme_config[$text]) ? $this->theme_config[$text] : ''), ENT_QUOTES, XcrudConfig::
            $mbencoding);
    }
    ...
    public static function load_css()
    {
        ...
        {
            $out .= '<link href="' . XcrudConfig::$scripts_url . '/' . XcrudConfig::$plugins_uri .
        ...
        $out .= '<link href="' . XcrudConfig::$scripts_url . '/' . XcrudConfig::$plugins_uri .
            '/timepicker/jquery-ui-timepicker-addon.css" rel="stylesheet" type="text/css" /><link href="' . XcrudConfig::$scripts_url .
            '/' . XcrudConfig::$themes_uri . '/' . XcrudConfig::$theme . '/xcrud.css" rel="stylesheet" type="text/css" />';
        return $out;
    }
    public static function load_js()
    {
        ...
        if (XcrudConfig::$load_jquery_ui)
            $out .= '<script src="' . XcrudConfig::$scripts_url . '/' . XcrudConfig::$plugins_uri .
        ...
        if ($language != 'en')
        {
            if (is_file(XCRUD_PATH . '/' . XcrudConfig::$lang_path . '/datepicker/jquery.ui.datepicker-' . $language . '.js'))
                $out .= '<script src="' . XcrudConfig::$scripts_url . '/' . XcrudConfig::$lang_uri .
    ...
    protected function send_http_request($url, $data, $method, $return_result = false)
    {
        ...
        {
            case 'get':
                $ch = curl_init($path . ((mb_strpos($path, '?', 0, XcrudConfig::$mbencoding) === false) ? '?' : '&') . $data);
                break;
            case 'post':
                $ch = curl_init($path);
        ...
        if ($this->result_list)
        {
            foreach ($this->result_list as $key => $row)
            {
                ...
                if (($this->is_edit || $this->is_remove || $this->is_view || $this->buttons || $this->is_duplicate || $this->
                    grid_restrictions) && $this->task != 'print' && $this->buttons_position == 'right')
                {
                    $out .= $this->open_tag($item, 'xcrud-actions' . (XcrudConfig::$fixed_action_buttons ? ' xcrud-fix' : ''), $this->
    ...
    protected function render_control_fields()
    {
        ...
        $out .= $this->single_tag($tag, '', array('name' => 'limit', 'value' => ($this->limit ? $this->limit : XcrudConfig::$limit)));
    ...
    {
        if ($time)
            return $utc ? gmdate(XcrudConfig::$php_date_format, $time) : date(XcrudConfig::$php_date_format, $time);
        else
            return '';
    }
    ...
    {
        if ($time)
            return $utc ? gmdate(XcrudConfig::$php_date_format . ' ' . XcrudConfig::$php_time_format, $time) : date(XcrudConfig::
                $php_date_format . ' ' . XcrudConfig::$php_time_format, $time);
        else
            return '';
    }
    ...
    {
        if ($time)
            return $utc ? gmdate(XcrudConfig::$php_time_format, $time) : date(XcrudConfig::$php_time_format, $time);
        else
            return '';
    }
